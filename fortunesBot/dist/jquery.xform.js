function SinglexState(input){
    this.input = input;
    this.answer = '';
    this.next = false;
    return this;
};
SinglexState.prototype.hasNext = function(){
    return this.next;
};
function xState(wrapper, SinglexState, form, params, originalFormHtml) {
    this.form = form;
    this.wrapper = wrapper;
    this.current = SinglexState;
    this.answers = {};
...
xState.prototype.destroy = function(){
...
xState.prototype.newState = function(options) {
    ...
    return new SinglexState(input);
};
xState.prototype.next = function(){
...
xState.prototype.printQuestion = function(){
...
xState.prototype.printAnswers = function(answers, multiple){
    var opened = false;
    ...
    if(this.parameters.selectInputStyle=='disable') {
        ...
        if(!this.current.input.multiple) {
            ...
            $(this.wrapper).find('#xForm button').css({display: 'none'});
        } else {
            $(this.wrapper).find('#'+this.parameters.inputIdName).prop('disabled', true);
            $(this.wrapper).find('#'+this.parameters.inputIdName).attr('placeholder', this.parameters.selectInputDisabledText);
        }
...
xState.prototype.answerWith = function(answerText, answerObject) {
    ...
    if(this.current.input.type=='select' && this.parameters.selectInputStyle=='disable') {
        ...
        if(!this.current.input.multiple) {
            ...
            $(this.wrapper).find('#xForm button').css({display: 'block'});
        } else {
            $(this.wrapper).find('#'+this.parameters.inputIdName).prop('disabled', false);
            $(this.wrapper).find('#'+this.parameters.inputIdName).attr('placeholder', this.parameters.placeHolder);
        }
...
(function($){
    $.fn.xform = function(options){
        ...
        var parameters = $.extend(true, {}, {
            ...
            eventList : {
                onSubmitForm : function(xState) {
                    console.log('completed');
                    xState.form.submit();
                    return true;
                },
                onInputSubmit : function(xState, readyCallback) {
                    if(xState.current.input.hasOwnProperty('callback')) {
                        if(typeof xState.current.input.callback === 'string') {
                            window[xState.current.input.callback](xState, readyCallback);
                        } else {
                            xState.current.input.callback(xState, readyCallback);
                        }
                    } else {
                        readyCallback();
                    }
        ...
        */
        var inputs = $(this).find('input, select, textarea').map(function(){
            ...
            input['questions'] = $(this).attr('data-x-question').split("|");
            ...
            if($(this).parent('div[data-x-case]').length) {
                input['case'] = $(this).parent('div[data-x-case]').attr('data-x-case');
                input['fork'] = $(this).parent('div[data-x-case]').parent('div[data-x-fork]').attr('data-x-fork');
            }
            input['element'] = this;
            $(this).detach();
            return input;
        ...
        if(inputs.length) {
            ...
            switch(parameters.typeInputUi) {
                case 'input':
                    inputForm = $('<form id="' + parameters.formIdName + '" class="xFormDynamic"><div class="options dragscroll"></div><input id="' + parameters.inputIdName + '" type="text" placeholder="'+ parameters.placeHolder +'" class="userInputDynamic"></><button type="submit" class="submit">'+parameters.buttonText+'</button><span class="clear"></span></form>');
                    break;
                case 'textarea':
                    inputForm = $('<form id="' + parameters.formIdName + '" class="xFormDynamic"><div class="options dragscroll"></div><textarea id="' + parameters.inputIdName + '" rows="1" placeholder="'+ parameters.placeHolder +'" class="userInputDynamic"></textarea><button type="submit" class="submit">'+parameters.buttonText+'</button><span class="clear"></span></form>');
                    break;
                default :
